...
def _chair(state: str = "live", holder: str = "loop-a") -> dict:
    return {"holder": holder, "state": state, "minutes_ago": 12}


def test_a_run_whose_launched_by_differs_from_the_live_chair_is_orphaned():
    r = runs_top.row("r1", True, [], [], [], None, "loop-b", _chair())
    assert r.status == "orphaned"
    assert runs_top.highlight(r) == "alert"


def test_a_run_matching_the_live_chairs_holder_is_not_orphaned():
    r = runs_top.row("r1", True, [], [], [], None, "loop-a", _chair())
    assert r.status == "running"


def test_a_run_is_orphaned_when_the_chair_is_stale():
    r = runs_top.row("r1", True, [], [], [], None, "loop-a", _chair("stale"))
    assert r.status == "orphaned"


def test_a_run_is_orphaned_when_the_chair_is_crashed():
    r = runs_top.row("r1", True, [], [], [], None, "loop-a", _chair("crashed"))
    assert r.status == "orphaned"


def test_no_chair_file_is_not_an_alert():
    r = runs_top.row("r1", True, [], [], [], None, "", None)
    assert r.status == "running"


def test_the_chair_line_and_by_column_render_from_literals():
    rows = [runs_top.row("r1", True, [], [], [], None, "loop-b", _chair("stale"))]
    lines = runs_top.render(rows, 80, _chair("stale"))
    assert lines[0] == "chair: loop-a (stale, beat 12m ago)"
    assert "BY" in lines[1]
    assert lines[2].rstrip().endswith("loop-b")


def test_render_with_no_chair_arg_omits_the_chair_line():
    lines = runs_top.render([], 40)
    assert lines[0].startswith("RUN")


def test_a_stale_chair_line_highlights_alert_a_live_one_does_not():
    assert runs_top.chair_highlight(_chair("stale")) == "alert"
    assert runs_top.chair_highlight(_chair("crashed")) == "alert"
    assert runs_top.chair_highlight(_chair("live")) == "normal"
    assert runs_top.chair_highlight(None) == "normal"


def test_chair_notifications_fires_only_on_a_live_to_stale_or_none_transition_with_a_live_run():
...
def test_a_chair_transition_from_live_to_crashed_with_a_live_run_notifies(tmp_path):
    ...
    calls = []
    alive_flags = iter([True, False])  # the chair's own pid, live then dead
...
def test_cli_runs_top_once_uses_the_resolved_heartbeat_minutes(monkeypatch, tmp_path, capsys):
    ...
    assert rc == 0
    assert "chair: loop-a (stale, beat 5m ago)" in capsys.readouterr().out
